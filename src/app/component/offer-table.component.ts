import {
    Component,
    ElementRef, OnInit, OnChanges, SimpleChanges
} from '@angular/core';
...
export class OfferTableComponent implements OnInit, OnChanges {
    ...
    private fields = [
        {
            id: 'stageCode', label: 'Стадия', visible: true, displayFor: OfferSource.LOCAL, sort: 0, val: (ofr: Offer) => {
            return ofr.stageCode;
        }
        },
        {
            id: 'photo', label: 'Фото', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            //return ofr.main_photo_thumbnail;
            return '';
        }
        },
        {
            id: 'typeCode', label: 'Тип', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return this.typeCodeOptions[ofr.typeCode];
        }
        },
        {
            id: 'locality', label: 'Нас. пункт', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return ofr.fullAddress.city;
        }
        },
        {
            id: 'district', label: 'Район', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return ofr.district;
        }
        },
        {
            id: 'poi', label: 'Ориентир', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return ofr.poi;
        }
        },
        {
            id: 'address', label: 'Адрес', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return ofr.fullAddress.street;
        }
        },
        {
            id: 'roomsCount', label: 'Комнаты', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            var res = '';
        ...
        {
            id: 'apScheme', label: 'Планировка', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return this.apSchemaOptions[ofr.apSchemeId];
        }
        },
        {
            id: 'houseType', label: 'Материал', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return this.houseTypeOptions[ofr.houseTypeId];
        }
        },
        {
            id: 'floor', label: 'Этаж', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            var res = '';
            if (ofr.floor) {
                res += ofr.floor;
            }
        ...
        {
            id: 'squareTotal', label: 'Площадь', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
                return ofr.squareTotal;
            }
        },
        {
            id: 'ownerPrice', label: 'Цена', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
                return ofr.ownerPrice;
            }
        },
        {
            id: 'priceSq', label: 'Цена м2', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
        ...
        {
            id: 'mls', label: 'MLS', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
                return '';
            }
        },
        {
            id: 'sourceMedia', label: 'Источник', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
                return {media: ofr.sourceMedia, url: ofr.sourceUrl};
        }
        },
        {
            id: 'mediator', label: 'Предложение', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
        ...
        {
            id: 'personName', label: 'Контакт', visible: true, displayFor: "", sort: 0, width: "110px", val: (ofr: Offer) => {
                if (ofr.person)
        ...
        {
            id: 'personType', label: 'Тип контакта', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
                if (ofr.person) return this.personType[ofr.person.typeCode];
                else return '';
            }
        },
        {
            id: 'agentName', label: 'Ответственный', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
                if (ofr.agent) return ofr.agent;
                else return '';
            }
        },
        {
            id: 'manager', label: 'Менеджер', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return '~';
        }
        },

        {
            id: 'reqests', label: 'Заявки', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return '10';
        }
        },
        {
            id: 'click_count', label: 'Кол-во кликов', visible: false, displayFor: "", sort: 0, val: (ofr: Offer) => {
            return '100';
        }
        },
        {
            id: 'progress', label: 'Прогресс', visible: false, displayFor: OfferSource.LOCAL, sort: 0, val: (ofr: Offer) => {
            return '50%';
        }
        },

        {
            id: 'addDate', label: 'Добавлено', visible: true, displayFor: "", sort: 0, val: (ofr: Offer) => {
            if (ofr.addDate) return moment(ofr.addDate * 1000).format('DD.MM.YY HH:mm');
            return "";
        }
        },
        {
            id: 'assignDate', label: 'Назначено', visible: false, displayFor: OfferSource.LOCAL, sort: 0, val: (ofr: Offer) => {
                if (ofr.assignDate) return moment(ofr.assignDate * 1000).format('DD.MM.YY HH:mm');
                else return "";
            }
        },
        {
            id: 'changeDate', label: 'Изменено', visible: false, displayFor: OfferSource.LOCAL, sort: 0, val: (ofr: Offer) => {
                if (ofr.changeDate) return moment(ofr.changeDate * 1000).format('DD.MM.YY HH:mm');
                else return "";
            }
        },
        {
            id: 'lastSeenDate', label: 'Актуально', visible: true, displayFor: OfferSource.LOCAL, sort: 0, val: (ofr: Offer) => {
    ...
    getPnone(ph_bl){
        return PhoneBlock.getNotNullPhone(ph_bl);
    }
}
